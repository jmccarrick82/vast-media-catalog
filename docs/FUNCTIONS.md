...
**Steps:** ffmpeg scene filter → per-shot midpoint keyframe extract → classify each frame with `nvidia/llama-3.2-11b-vision-instruct` → escalate borderline-confidence frames (`low ≤ conf < high`, configurable) to `nvidia/llama-3.2-90b-vision-instruct` → keep matches ≥ `escalation_confidence_low` → merge adjacent matches within `merge_gap_seconds` → apply `min_clip_seconds` / `max_clip_seconds` constraints → **LLM curation** picks the top `max_clips_per_source` (default 5) when over-supplied → expand by editorial buffer (`clip_buffer_pre_seconds` / `clip_buffer_post_seconds`, default 0; clamped to source duration) → cut via `clips.cut_clip` (stream-copy by default for speed; override for frame-exact cuts) → upload each clip to `s3://james-media-clips/<source_id>/clip-NNN.mp4` with S3 metadata → emit `_ready.json` sentinel that fires Phase 3.

**Curation (v6):** Per-shot vision confidence is noisy on a single keyframe — when 17 shots all hit "high confidence: cartoon rabbit on screen", picking by raw confidence gives 17 nearly-identical bunny shots. The curation pass calls a text-only LLM (`nvidia/llama-3.3-70b-instruct` by default) with each candidate's per-shot reason + timestamp + confidence, and asks it to pick the best `max_clips_per_source` considering match quality, content diversity, and chronological spread across the source. Returns the chosen candidates in source order. Knobs: `curation_enabled` (default true), `curation_model`, `curation_timeout_seconds` (default 90s), `curation_retries`, `curation_diversity_weight` (`low`/`medium`/`high`, default `high`). Falls back to top-K-by-confidence on any failure so the pipeline never stalls on curation problems.
