...
# ---------------------------------------------------------------------------
# FUNCTION CONFIGS — runtime-editable knobs for every non-AI function
# ---------------------------------------------------------------------------
# One row per (scope, key). The value + default are stored JSON-encoded so the
# same table can hold numbers, bools, arrays, and nested objects without a
# schema migration. `value_type` drives both runtime coercion (shared/config.py)
# and the widget choice in the /settings UI.
#
# Examples of rows this table will hold:
#   scope="qc-inspector"  key="black_frame_max_ratio_fail"  value="0.5"   type="percent"
#   scope="qc-inspector"  key="video_codec_allowlist"       value='["h264","hevc"]'  type="json"
#   scope="subclipper"    key="default_ffmpeg_preset"       value='"fast"'  type="string"
#
# ---------------------------------------------------------------------------
# SOURCE_VIDEOS — spine of a pre-ingest run (QC → clip → package)
# ---------------------------------------------------------------------------
# One row per raw upload dropped in s3://james-media-inbox/. Updated by each
# pipeline stage as it runs:
#   qc-inspector   sets qc_* fields, moves file to qc-passed|qc-failed
#   ai-clipper     sets clip_* fields (Phase 2)
#   media-packager sets packaging_* + package_id fields (Phase 3)
#
# Lookups by source_id (md5 of inbox s3 path) or by s3_inbox_path. The
# webapp's ingest dossier view will join from here to extracted_clips
# and delivery_packages tables (added in Phase 2/3).

SOURCE_VIDEOS_SCHEMA = pa.schema([
    # Identity
    pa.field("source_id",                 pa.string()),   # PK: md5(s3_inbox_path)
    pa.field("s3_inbox_path",             pa.string()),   # original inbox location
    pa.field("filename",                  pa.string()),
    pa.field("file_size_bytes",           pa.int64()),
    pa.field("sha256",                    pa.string()),
    pa.field("uploaded_at",               pa.float64()),
    pa.field("uploaded_by",               pa.string()),

    # Probed technical metadata (from ffprobe)
    pa.field("duration_seconds",          pa.float64()),
    pa.field("video_codec",               pa.string()),
    pa.field("video_profile",             pa.string()),
    pa.field("width",                     pa.int32()),
    pa.field("height",                    pa.int32()),
    pa.field("fps",                       pa.float64()),
    pa.field("pixel_format",              pa.string()),
    pa.field("color_space",               pa.string()),
    pa.field("color_range",               pa.string()),
    pa.field("bitrate_total",             pa.int64()),
    pa.field("audio_codec",               pa.string()),
    pa.field("audio_channels",            pa.int32()),
    pa.field("audio_sample_rate",         pa.int32()),
    pa.field("audio_layout",              pa.string()),

    # QC results — populated by qc-inspector
    pa.field("qc_status",                 pa.string()),   # pending|passed|warn|failed
    pa.field("qc_verdict_reason",         pa.string()),   # first reason or "all checks clean"
    pa.field("qc_issues_json",            pa.string()),   # JSON list of all issues found (for warn/fail)
    pa.field("qc_black_runs_json",        pa.string()),   # JSON list of {start,end}
    pa.field("qc_freeze_runs_json",       pa.string()),
    pa.field("qc_silence_runs_json",      pa.string()),
    pa.field("qc_black_ratio",            pa.float64()),  # fraction of duration that's black
    pa.field("qc_freeze_ratio",           pa.float64()),
    pa.field("qc_silence_ratio",          pa.float64()),
    pa.field("qc_loudness_lufs",          pa.float64()),  # integrated loudness
    pa.field("qc_true_peak_dbtp",         pa.float64()),
    pa.field("qc_is_vfr",                 pa.bool_()),
    pa.field("qc_is_interlaced",          pa.bool_()),
    pa.field("qc_config_snapshot_json",   pa.string()),   # which thresholds were used (for audit)
    pa.field("qc_checked_at",             pa.float64()),
    pa.field("qc_elapsed_seconds",        pa.float64()),

    # Clip extraction (Phase 2)
    pa.field("clip_prompt",               pa.string()),
    pa.field("clip_prompt_source",        pa.string()),
    pa.field("clip_extraction_status",    pa.string()),
    pa.field("clip_count",                pa.int32()),
    pa.field("clip_extracted_at",         pa.float64()),

    # Packaging (Phase 3)
    pa.field("packaging_status",          pa.string()),
    pa.field("package_id",                pa.string()),
    pa.field("packaged_at",               pa.float64()),
    pa.field("catalog_handoff_at",        pa.float64()),

    # Lifecycle
    pa.field("status",                    pa.string()),   # active|quarantined|archived
    pa.field("current_s3_path",           pa.string()),   # where the file is now (after move)
    pa.field("created_at",                pa.float64()),
...
# ---------------------------------------------------------------------------
# EXTRACTED CLIPS (Phase 2 — ai-clipper)
# Many-rows-per-source. Each row is one AI-selected clip cut out of a
# source_videos row that passed QC. Populated by james-ai-clipper.
# ---------------------------------------------------------------------------
EXTRACTED_CLIPS_SCHEMA = pa.schema([
    # Identity
    pa.field("clip_id",               pa.string()),   # PK: md5(clip_s3_path)
    pa.field("source_id",             pa.string()),   # FK → source_videos.source_id
    pa.field("clip_index",            pa.int32()),    # 0-based order within the source

    # Location / media
    pa.field("clip_s3_path",          pa.string()),
    pa.field("start_seconds",         pa.float64()),
    pa.field("end_seconds",           pa.float64()),
    pa.field("duration_seconds",      pa.float64()),
    pa.field("shot_count",            pa.int32()),    # # adjacent matching shots merged
    pa.field("file_size_bytes",       pa.int64()),

    # Prompt & match
    pa.field("prompt",                pa.string()),   # what we asked the vision model
    pa.field("prompt_source",         pa.string()),   # s3_metadata|sidecar|default
    pa.field("match_confidence",      pa.float64()),  # max confidence across frames
    pa.field("match_reason",          pa.string()),   # vision model's explanation
...
# ---------------------------------------------------------------------------
# DELIVERY PACKAGES (Phase 3 — media-packager)
# One row per source video's delivery bundle. A package wraps one source's
# clips in every configured rendition + thumbnail + manifest + C2PA.
# ---------------------------------------------------------------------------
DELIVERY_PACKAGES_SCHEMA = pa.schema([
    pa.field("package_id",             pa.string()),   # PK: uuid4 hex
    pa.field("source_id",              pa.string()),   # FK → source_videos.source_id

    pa.field("package_root_s3_path",   pa.string()),   # s3://.../deliveries/<package_id>/
    pa.field("manifest_s3_path",       pa.string()),   # s3://.../manifest.json
    pa.field("clip_count",             pa.int32()),
    pa.field("rendition_count",        pa.int32()),
    pa.field("total_size_bytes",       pa.int64()),

    # Licensing (from config defaults, or overridden by uploader S3 metadata)
    pa.field("rights_cleared_for_json", pa.string()),  # JSON list of strings
    pa.field("restrictions_json",       pa.string()),  # JSON list of strings
    pa.field("source_attribution",      pa.string()),
    pa.field("clearance_expires_at",    pa.float64()), # epoch seconds
    pa.field("licensing_notes",         pa.string()),

    # C2PA provenance
    pa.field("c2pa_enabled",            pa.bool_()),
    pa.field("c2pa_signer",             pa.string()),  # CN of signing cert
    pa.field("c2pa_claim_generator",    pa.string()),
    pa.field("c2pa_signed_count",       pa.int32()),   # how many renditions got signed
    pa.field("c2pa_errors_json",        pa.string()),  # JSON list of per-rendition errors

    # Hand-off
    pa.field("delivered_to",            pa.string()),
    pa.field("notified_at",             pa.float64()),

    # Lifecycle
    pa.field("status",                  pa.string()),  # building|ready|delivered|expired|failed
...
# ---------------------------------------------------------------------------
# PACKAGE RENDITIONS (Phase 3 — media-packager)
# Many-per-package. One row per (clip × rendition preset) pair.
# ---------------------------------------------------------------------------
PACKAGE_RENDITIONS_SCHEMA = pa.schema([
    pa.field("rendition_id",           pa.string()),   # PK: md5(rendition_s3_path)
    pa.field("package_id",             pa.string()),   # FK → delivery_packages.package_id
    pa.field("clip_id",                pa.string()),   # FK → extracted_clips.clip_id
    pa.field("source_id",              pa.string()),   # FK (denormalized for easy query)

    pa.field("rendition_name",         pa.string()),   # e.g. "h264-1080p"
    pa.field("container",              pa.string()),   # e.g. "mp4"
    pa.field("video_codec",            pa.string()),
    pa.field("audio_codec",            pa.string()),
    pa.field("width",                  pa.int32()),
    pa.field("height",                 pa.int32()),
    pa.field("fps",                    pa.float64()),
    pa.field("video_bitrate",          pa.int32()),
    pa.field("audio_bitrate",          pa.int32()),

    pa.field("rendition_s3_path",      pa.string()),
    pa.field("file_size_bytes",        pa.int64()),
    pa.field("sha256",                 pa.string()),   # computed post-signing

    # C2PA provenance (per rendition — signatures are per-file)
    pa.field("c2pa_signed",            pa.bool_()),
    pa.field("c2pa_manifest_label",    pa.string()),   # urn:uuid:... from c2patool
    pa.field("c2pa_signer",            pa.string()),
    pa.field("c2pa_signed_at",         pa.float64()),
    pa.field("c2pa_error",             pa.string()),   # non-null if signing failed

    # Lifecycle
    pa.field("status",                 pa.string()),   # ready|failed|superseded
    pa.field("created_at",             pa.float64()),
    pa.field("updated_at",             pa.float64()),
])


FUNCTION_CONFIGS_SCHEMA = pa.schema([
    pa.field("config_id",     pa.string()),   # PK: f"{scope}:{key}"
    pa.field("scope",         pa.string()),   # e.g., "qc-inspector", "subclipper", "global"
    pa.field("key",           pa.string()),   # short name of the setting
    pa.field("value",         pa.string()),   # JSON-encoded current value
    pa.field("value_type",    pa.string()),   # int|float|bool|string|duration_seconds|percent|db|json
    pa.field("default_value", pa.string()),   # JSON-encoded factory default
    pa.field("description",   pa.string()),   # human explanation for UI help text
    pa.field("min_value",     pa.string()),   # optional UI range bound (JSON-encoded)
    pa.field("max_value",     pa.string()),   # optional UI range bound (JSON-encoded)
    pa.field("ui_group",      pa.string()),   # e.g., "Black frames", "Loudness"
    pa.field("ui_order",      pa.int32()),    # sort order within a group
    pa.field("updated_at",    pa.float64()),  # last-modified epoch
    pa.field("updated_by",    pa.string()),   # who/what last wrote it
])
